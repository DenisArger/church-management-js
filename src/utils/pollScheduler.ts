...
export const shouldSendYouthReportFollowUpReminder = (
  ...
  if (isWithinMoscowTimeWindow(currentTime, { hour: 12, minute: 25 })) {
 ...
 * Target time: 07:00 (Europe/Moscow) every day, 15-minute window after target.
 */
export const shouldSendDailyScripture = (
  currentTime: Date = new Date(),
): boolean => {
  if (isWithinMoscowTimeWindow(currentTime, { hour: 7, minute: 0 })) {
