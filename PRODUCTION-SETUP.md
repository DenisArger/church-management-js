# 🚀 Настройка бота на продакшен

Это руководство поможет вам настроить бота для работы в продакшене.

## Быстрая настройка

### 1. Автоматическая настройка локального окружения

```bash
yarn setup:production
```

Или вручную:

```bash
./scripts/setup-production.sh
```

Этот скрипт:
- Создаст или обновит файл `.env`
- Установит `NODE_ENV=production`
- Установит `DEBUG=false`
- Настроит уровень логирования для продакшена
- Проверит наличие всех обязательных переменных

### 2. Настройка переменных окружения в Netlify

**Важно**: Переменные окружения нужно настроить в Netlify Dashboard, так как они используются при выполнении функций.

#### Вариант 1: Через Netlify Dashboard

1. Откройте [Netlify Dashboard](https://app.netlify.com)
2. Выберите ваш сайт
3. Перейдите в **Site settings** > **Environment variables**
4. Добавьте все переменные из вашего `.env` файла

#### Вариант 2: Через Netlify CLI

```bash
# Войдите в Netlify (если еще не вошли)
netlify login

# Установите переменные окружения
netlify env:set NODE_ENV production
netlify env:set DEBUG false
netlify env:set LOG_LEVEL info
netlify env:set LOG_FORMAT json
netlify env:set TELEGRAM_BOT_TOKEN "your_token_here"
netlify env:set ALLOWED_USERS "282850458,123456789"
netlify env:set NOTION_TOKEN "your_notion_token"
# ... и так далее для всех переменных
```

#### Обязательные переменные для продакшена

```env
NODE_ENV=production
DEBUG=false
LOG_LEVEL=info
LOG_FORMAT=json
TELEGRAM_BOT_TOKEN=your_production_bot_token
ALLOWED_USERS=282850458,123456789
NOTION_TOKEN=your_notion_token
NOTION_PRAYER_DATABASE=your_database_id
NOTION_GENERAL_CALENDAR_DATABASE=your_database_id
NOTION_DAILY_DISTRIBUTION_DATABASE=your_database_id
NOTION_WEEKLY_PRAYER_DATABASE=your_database_id
```

#### Опциональные переменные

```env
TELEGRAM_MAIN_CHANNEL_ID=your_channel_id
TELEGRAM_MAIN_GROUP_ID=your_group_id
TELEGRAM_YOUTH_GROUP_ID=your_youth_group_id
```

**Примечание**: Debug переменные (`TELEGRAM_BOT_TOKEN_DEBUG`, `TELEGRAM_CHAT_ID_DEBUG`, `TELEGRAM_TOPIC_ID_DEBUG`) не нужны для продакшена.

### 3. Деплой в Netlify

```bash
yarn deploy:full
```

Или вручную:

```bash
./scripts/deploy.sh
```

### 4. Настройка webhook

После успешного деплоя настройте webhook:

```bash
yarn webhook:set
```

Или вручную:

```bash
./scripts/webhook-manager.sh set-file
```

### 5. Проверка webhook

```bash
yarn webhook:info
```

## Проверка продакшен настроек

### Локальная проверка

Скрипт `setup-production.sh` автоматически проверяет:
- ✅ `NODE_ENV=production`
- ✅ `DEBUG=false`
- ✅ Наличие всех обязательных переменных

### Проверка в Netlify

1. Откройте Netlify Dashboard
2. Перейдите в **Site settings** > **Environment variables**
3. Убедитесь, что все переменные установлены
4. Проверьте, что `NODE_ENV=production` и `DEBUG=false`

### Проверка работы бота

1. Отправьте команду `/help` боту в Telegram
2. Проверьте логи в Netlify Dashboard:
   - **Functions** > выберите функцию > **Logs**
3. Убедитесь, что бот отвечает корректно

## Различия между Development и Production

| Параметр | Development | Production |
|----------|-------------|------------|
| `NODE_ENV` | `development` | `production` |
| `DEBUG` | `true` (опционально) | `false` |
| `LOG_LEVEL` | `debug` или `info` | `info` или `warn` |
| Бот токен | Может быть debug токен | Основной токен |
| Группы | Тестовые группы | Продакшен группы |
| Логирование | Подробное | Минимальное |

## Важные замечания

### Безопасность

- ✅ Никогда не коммитьте файл `.env` в репозиторий
- ✅ Используйте продакшен токены только в Netlify
- ✅ Регулярно обновляйте токены
- ✅ Ограничьте доступ к авторизованным пользователям

### Debug режим

В продакшене `DEBUG=false` означает:
- Используется основной бот токен (`TELEGRAM_BOT_TOKEN`)
- Сообщения отправляются в продакшен группы
- Debug бот не используется
- Все функции работают в полном режиме

### Логирование

В продакшене рекомендуется:
- `LOG_LEVEL=info` - для нормальной работы
- `LOG_LEVEL=warn` - для минимального логирования
- `LOG_FORMAT=json` - для удобного парсинга логов

## Устранение неполадок

### Бот не отвечает

1. Проверьте webhook: `yarn webhook:info`
2. Проверьте логи в Netlify Dashboard
3. Убедитесь, что переменные окружения установлены в Netlify

### Используется debug бот вместо продакшен

1. Проверьте `DEBUG=false` в Netlify переменных окружения
2. Проверьте `NODE_ENV=production` в Netlify переменных окружения
3. Перезапустите функции (redeploy)

### Ошибка "Missing required environment variables"

1. Проверьте, что все обязательные переменные установлены в Netlify
2. Убедитесь, что переменные не пустые
3. Проверьте правильность имен переменных

## Дополнительные ресурсы

- [Документация по деплою](DEPLOYMENT.md)
- [Конфигурация](docs/configuration.md)
- [Netlify Functions](docs/netlify-functions.md)

---

**Следующие шаги после настройки продакшена:**

1. ✅ Протестируйте все команды бота
2. ✅ Проверьте работу автоматических опросов
3. ✅ Настройте мониторинг логов
4. ✅ Сообщите пользователям о запуске

