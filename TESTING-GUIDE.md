# Тестирование Telegram бота

## Настройка завершена! ✅

### Конфигурация тестового бота:

- **Токен бота**: `TELEGRAM_BOT_TOKEN_DEBUG` (настроен в .env)
- **Chat ID**: `-1002317620302` (группа для тестирования)
- **Topic ID**: `78` (топик для тестирования)
- **Вебхук URL**: `https://f5fa2ef2--church-telegram-bot.netlify.live/.netlify/functions/telegram-webhook`

### Статус серверов:

- ✅ **Netlify dev сервер**: `http://localhost:8889` (работает)
- ✅ **Отладочный сервер**: `http://localhost:3000` (работает)
- ✅ **Netlify Live**: `https://f5fa2ef2--church-telegram-bot.netlify.live` (настроен)

### Команды для управления:

#### Проверка статуса вебхука:

```bash
bash test-bot-webhook.sh info
```

#### Тестирование вебхука:

```bash
bash test-bot-webhook.sh test https://f5fa2ef2--church-telegram-bot.netlify.live/.netlify/functions/telegram-webhook
```

#### Удаление вебхука (если нужно):

```bash
bash test-bot-webhook.sh delete
```

#### Перенастройка вебхука:

```bash
bash test-bot-webhook.sh set-netlify https://f5fa2ef2--church-telegram-bot.netlify.live
```

### Тестирование команд бота:

Теперь вы можете тестировать бота в Telegram группе `-1002317620302` в топике `78`. Доступные команды:

- `/help` - справка по командам
- `/prayer` - добавить молитвенную просьбу
- `/schedule` - расписание на неделю
- `/sunday` - информация о воскресном служении
- `/daily` - ежедневное писание
- `/youth` - создание опроса для молодежи
- `/debug` - отладочные команды

### Скрипты тестов (из корня проекта):

- `yarn test` — сборка + **test-calendar** (экспорты, formatServiceInfo, getSundayMeeting, getWeeklySchedule)
- `yarn test:poll` — сборка + test-auto-poll (text, scheduler)
- `yarn test:integration` — полный прогон: test-calendar, test-auto-poll, test-youth-poll (нужны Notion, Telegram)
- `bash scripts/test/test-debug.sh http://localhost:3000` — /health и /webhook (перед этим: `yarn debug:server`)

Подробнее: [scripts/test/README.md](scripts/test/README.md).

### Логи и отладка:

#### Просмотр логов Netlify dev:

Логи отображаются в терминале где запущен `netlify dev --live`

#### Тестирование локально:

```bash
# Тест отладочного сервера
node test-webhook.js

# Тест Netlify функции
curl -X POST http://localhost:8889/.netlify/functions/telegram-webhook \
  -H "Content-Type: application/json" \
  -d '{"update_id": 1, "message": {"message_id": 1, "from": {"id": 123456789, "first_name": "Test"}, "chat": {"id": 123456789, "type": "private"}, "text": "/help", "date": 1234567890}}'
```

### Устранение проблем:

1. **502 Bad Gateway**:

   - Убедитесь, что Netlify dev сервер запущен: `netlify dev --live`
   - Проверьте, что порт 8889 свободен

2. **Webhook не отвечает**:

   - Проверьте статус: `bash test-bot-webhook.sh info`
   - Перезапустите вебхук: `bash test-bot-webhook.sh set-netlify https://f5fa2ef2--church-telegram-bot.netlify.live`

3. **Бот не отвечает в группе**:
   - Убедитесь, что бот добавлен в группу
   - Проверьте, что используется правильный токен тестового бота
   - Проверьте права бота в группе

### Файлы для отладки:

- `test-bot-webhook.sh` - управление вебхуком тестового бота
- `test-webhook.js` - тестирование вебхука
- `debug-server.ts` - отладочный сервер
- `.env` - конфигурация (токены, ID групп)

Теперь тестовый бот готов к работе! 🚀
